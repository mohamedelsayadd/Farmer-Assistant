...
- Customer support is a second agent, `support_agent` ("Customer Support Agent", also in `agent/agent.py`), reached from `farmer_agent` by an SDK **handoff** (`handoffs=[support_agent]`, tool `transfer_to_customer_support_agent`), and hands back with `support_agent.handoffs = [farmer_agent]` (tool `transfer_to_farmer_assistant`) for anything outside the support flow. Never wire it with `as_tool`. It shares the model/settings, uses `SUPPORT_PROMPT` from `src/agent/support_prompts.py` (kept separate from the farmer's `src/agent/prompts.py`) + today's date (`support_prompt()`), and its only tool is `SUPPORT_TOOLS = [get_devices_status]`.
- The farmer hands off only when the user explicitly reports a device, sensor, connectivity, package, or account problem, or answers the support agent's last question in an already-started support conversation (judged from history). Stale/abnormal tool results never trigger a handoff; readings requests stay with the farmer. Each request starts at the agent that wrote the last assistant turn (`starting_agent(history)`, from the `agent` name stored on that turn; `farmer_agent` when untagged), so support follow-ups stay with support without a new handoff.
- `get_devices_status` returns per device `_id`, `name`, `last_reading_time`, `readings`, `connection_type` (`WIFI`/`4G`), `renewal_type` (`automatic`/`manual`, 4G only) and `renewal_date` (manual only). `ReNileClient.get_devices_status` currently returns **dummy data** with no HTTP call, until the backend API exists. It is not cached.
- The agent runs **statelessly** per request: no SDK session. `ChatService.chat` builds `AgentContext`, converts Redis history into `{role, content}` input items with `build_messages`, and calls `Runner.run(starting_agent(history), ..., context=context)`; the assistant reply is stored with `agent=result.last_agent.name`. Tests inject a fake model with `ChatService(run_config=RunConfig(model=...))`.
...
- Prompt/date/language/scope changes (including `SUPPORT_PROMPT` and the handoff rules): update `tests/test_agent_memory_context.py`. Handoff/support orchestration: `tests/test_agent.py`.
