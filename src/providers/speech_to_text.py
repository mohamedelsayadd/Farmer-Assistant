...
class SpeechToTextProvider:
    ...
    def _transcribe_wav_sync(self, audio_bytes: bytes) -> str:
        ...
        try:
            ...
            logger.info("speech_to_text_completed audio_bytes=%s text_chars=%s latency_ms=%s", len(audio_bytes), len(text), elapsed_ms)
            return text
        except Exception as exc:
            elapsed_ms = int((perf_counter() - started_at) * 1000)
            logger.exception("speech_to_text_failed audio_bytes=%s latency_ms=%s", len(audio_bytes), elapsed_ms)
            ...
            if path:
                try:
                    ...
                    logger.warning("speech_to_text_temp_file_cleanup_failed")
    ...
    def _get_model(self):
        if self._model is None:
            ...
            logger.info(
                "speech_to_text_model_loading model=%s device=%s compute_type=%s",
                self._model_name,
                self._device,
                self._compute_type,
            )
            self._model = WhisperModel(self._model_name, device=self._device, compute_type=self._compute_type)
            logger.info("speech_to_text_model_loaded model=%s", self._model_name)
        return self._model
