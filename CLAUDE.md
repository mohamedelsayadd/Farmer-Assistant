...
**Two Redis databases, deliberately separate.** DB 0 (`memory/redis_memory.py`) holds only `user`/`assistant` turns under `conversation:{id}` (assistant turns also store the answering agent's name, which `build_messages` never passes to the model); DB 1 (`memory/tool_cache.py`) holds tool results under `tool_cache:{conversation_id}:{tool}:{args_hash}`. Tool results are never written into prompt memory — the model only sees them within the round that fetched them. One exception: `get_devices_status` always fetches fresh but saves its result to DB 1, and the async `support_instructions` re-shows it at the end of the support prompt, so later support turns continue the flow from the real `connection_type`/`renewal_*`/`readings` instead of guessing.
